...
# Runner images can carry harmless local core edits (observed: rustup.rb). This is an ephemeral
# GitHub-hosted checkout only: require that no untracked files exist, then restore tracked files
# from its current official commit before switching to the workflow-pinned official commit.
[[ -z "$(/usr/bin/git -C "$core_dir" status --porcelain --untracked-files=normal | /usr/bin/awk '$1=="??"{print}')" ]] || { echo 'Unexpected untracked files in runner core checkout' >&2; exit 1; }
/usr/bin/git -C "$core_dir" restore --source=HEAD --staged --worktree -- .
...
for old_dir in /usr/local/Cellar /usr/local/opt /usr/local/var/homebrew/linked;do if [[ -e "$old_dir" || -L "$old_dir" ]];then /bin/mv "$old_dir" "$backup_dir/$(basename "$old_dir")";fi;/bin/mkdir -p "$old_dir";done
