...
try:
    ...
    for path in [ROOT/'cmd/brew-intel',ROOT/'scripts/prepare-runner.sh',ROOT/'scripts/retry-fetch.sh',ROOT/'scripts/verify-homebrew-pins.sh']:subprocess.run(['bash','-n',str(path)],check=True)
