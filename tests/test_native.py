...
from intelbrew.ci import BOTTLE_OPTIONS, transient_builds
...
class NativeTests(unittest.TestCase):
    ...
    def test_ci_metadata_preserves_build_and_test_dependency_roles(self):
        # API metadata splits dual roles into separate edges; local Ruby
        # recipes used in CI keep both tags on one Dependency object.
        item = native({"mode": "inspect", "names": ["wasi-libc"]}, ci=True)["wasi-libc"]
        for name in ("lld", "llvm"):
            self.assertIn(name, item["build"])
            self.assertIn(name, item["test"])
        plan = {"roots": ["wasi-libc"], "nodes": {
            "wasi-libc": {**item, "provider": "build"},
            "llvm": {"provider": "build", "runtime": [], "test": []},
            "lld": {"provider": "build", "runtime": ["llvm"], "test": []},
        }}
